...
public class ClipImageView extends ImageView implements View.OnTouchListener, ViewTreeObserver.OnGlobalLayoutListener{
     ...
     * @param clipHeight
     */
    public void borderAttrs(int clipWidth, int clipHeight){
...
//        if(drawableWidth <= drawableHeight){
        	...
        	if(drawableWidth < clipWidth){
        		baseMatrix.reset();
        		scale = (float)clipWidth / drawableWidth;
        		/**
        		 * 强制把缩放比例加上0.1，防止四舍五入导致放大比例不到位。只是这个项目第一次的获取的图片跟后面获取的图片大小不一样大，所以需要这样处理，别的项目不需要这句话。
        		 */
        		scale = scale + 0.1f;
        		// 缩放
        		baseMatrix.postScale(scale, scale);
        	}
        	if(drawableHeight < clipHeight){
        		baseMatrix.reset();
        		scale = (float)clipHeight / drawableHeight;
        		/**
        		 * 强制把缩放比例加上0.1，防止四舍五入导致放大比例不到位。只是这个项目第一次的获取的图片跟后面获取的图片大小不一样大，所以需要这样处理，别的项目不需要这句话。
        		 */
        		scale = scale + 0.1f;
    ...
    public Bitmap clip(){
    	...
    	draw(canvas);
    	return Bitmap.createBitmap(bitmap, (getWidth() - clipWidth) / 2, (getHeight() - clipHeight) / 2, clipWidth, clipHeight);
    }
}
