...
public class MobileAlbumActivity extends BaseActivity {
	...
	public void getType() {
		try {
			type = this.getIntent().getExtras().getString(TootooeNetApiUrlHelper.TYPE);
			activity_create_Id = this.getIntent().getExtras()
					.getString(TootooeNetApiUrlHelper.ACTIVITY_CREATE_ID);
			storyCreateId = this.getIntent().getExtras()
	...
	public void onitemclick(final List<MobileAlbumBean> mobileAlbumBeans,
		...
		justIsCommentOrWishBundle(photo_intent);
	...
	private void justIsCommentOrWishBundle(Intent photo_intent) {
		if(!TextUtils.isEmpty(activity_create_Id)){
			photo_intent.putExtra(TootooeNetApiUrlHelper.ACTIVITY_CREATE_ID, activity_create_Id);
			
		}
		if (!TextUtils.isEmpty(storyCreateId)) {
			photo_intent.putExtra(TootooeNetApiUrlHelper.STORY_CREATE_ID, storyCreateId);
		}
		if (!TextUtils.isEmpty(type)) {
			photo_intent.putExtra(TootooeNetApiUrlHelper.TYPE, type);
		}
	}
}
