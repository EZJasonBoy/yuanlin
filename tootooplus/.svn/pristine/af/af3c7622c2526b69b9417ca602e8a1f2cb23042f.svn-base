...
/**
 * 
 * @ClassName: GroupFreeCommentAdapter
 * @Description: 最新、最热
 * @author wuyulong
...
public class GroupFreeCommentAdapter extends BaseAdapter {
	...
	public GroupFreeCommentAdapter(Activity context,
			List<FreeCommentBean> parserResult) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		FreeCommentBean commentListBean = parserResult.get(position);
		CommentHolder commentHolder = null;
		if (convertView == null) {
			convertView = LayoutInflater.from(context).inflate(
					R.layout.free_comment_list_lv_item, null);
		...
		commentHolder.mLLActName
				.setOnClickListener(new MyOnClickActivityComment(
						commentListBean));
	...
	class MyOnClickActivityComment implements View.OnClickListener {
		...
		public MyOnClickActivityComment(FreeCommentBean commentListBean) {
		...
		public void onClick(View v) {
			...
			String activityId = commentListBean.getActivityId();
			String userId = commentListBean.getUserId();
			Bundle bundle = new Bundle();
	...
	public void setItemData(CommentHolder commentHolder,
			FreeCommentBean commentListBean) {
		...
		String commentLike=commentListBean.getCommentLike();
		
		String strUserName = commentListBean.getUserName();
		String strIsRecommend = commentListBean.getIsRecommend();
		String strActName = commentListBean.getActivityName();
		if (!TextUtils.isEmpty(strActName)) {
			commentHolder.mCtActName.setText("# "+strActName+" #");
		}
		if(!TextUtils.isEmpty(commentLike)){
			if(commentLike.equals("1")){//点击过
				Drawable goodClicked = TootooPlusApplication.getAppContext().getResources().getDrawable(R.drawable.icon_good_smallclicked);
				commentHolder.mCTLike.setCompoundDrawablesWithIntrinsicBounds(goodClicked, null, null, null);
			}else{
				Drawable noClicked = TootooPlusApplication.getAppContext().getResources().getDrawable(R.drawable.icon_good);
				commentHolder.mCTLike.setCompoundDrawablesWithIntrinsicBounds(noClicked, null, null, null);
			}
			
		}
		if (!TextUtils.isEmpty(strCountLike)) {
			commentHolder.mCTLike.setVisibility(View.VISIBLE);
			commentHolder.mCTLike.setText(strCountLike);// 设置点赞
		}else{
			commentHolder.mCTLike.setVisibility(View.GONE);
		}
		...
		if (!TextUtils.isEmpty(strIsRecommend) && strIsRecommend.equals("1")) {// 是否是推荐
																				// 1是推荐
