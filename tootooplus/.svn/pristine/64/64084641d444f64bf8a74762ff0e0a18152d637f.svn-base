			...
			case MSG_ACTION_CCALLBACK: {
				switch (msg.arg1) {
					case 1: {
						...
						if (resId > 0) {
							showNotification(context.getString(resId));
							if(!SharedPreferenceHelper.getLoginUserId(context).equals("") && !SharedPreferenceHelper.getLoginUserId(context).equals("0")){
								RequestParamsNet requestParamsNet = new RequestParamsNet();
								requestParamsNet.addQueryStringParameter(TootooeNetApiUrlHelper.SHARE_CALL_BACK_USERID, SharedPreferenceHelper.getLoginUserId(context));
								requestParamsNet.addQueryStringParameter(TootooeNetApiUrlHelper.SHARE_CALL_BACK_STORYID, TootooPlusApplication.shareStoryId);
								requestParamsNet.addQueryStringParameter(TootooeNetApiUrlHelper.SHARE_CALL_BACK_SOURCE, TootooPlusApplication.shareSource);
								CommonUtil.xUtilsGetSend(TootooeNetApiUrlHelper.SHARE_CALL_BACK_URL, requestParamsNet, new RequestCallBack<String>() {
