...
//			&&&&rectfourthree.length()>0&&
			if(defaulttypeStr.length()>0){
				defaulttypeStr.delete(defaulttypeStr.length() - 2, defaulttypeStr.length());
			}
			if(rectfourthree.length()>0){
				rectfourthree.delete(rectfourthree.length() - 2, rectfourthree.length());
			}
			if(squareOneToOne.length()>0){
				squareOneToOne.delete(squareOneToOne.length() - 2, squareOneToOne.length());
			}
					...
					requestParamsNet, new RequestCallBack<String>() {
						...
						private void setCreateListBean(final String yuantu,
							...
							if(mHashMap!=null){
								...
								if(insertPosition==ConstantsTooTooEHelper.NO_ITEM_CLICK_INSERT_CODE){
									...
									insertPosition(arr, rect, square, fourtheearr, onetonoearr, arrDefaultType, pageId, insertPosition);	
								}
						...
						*/
						private void insertBottom(String[] arr, String[] rect,
								String[] square, String[] fourtheearr,
								String[] onetonoearr, String[] arrDefaultType,
								String[] pageId) {
							...
							for (int i = 0; i < pageId.length; i++) {
								...
								if (!TextUtils.isEmpty(defaultTypeitem)&&!defaultTypeitem.equals("null")) {
								...
								if (listSize == 0) {
									...
									if (!TextUtils.isEmpty(defaultTypeitem)&&!defaultTypeitem.equals("null")) {//没办法后台竟然有时候会返回null的字符串
										int elementType = Integer
												.valueOf(defaultTypeitem);
										storyDetailListBean
												.setElementType(elementType);
									...
									if (heightItem != null) {
										...
										if (location == 1) {//
											...
											if (!TextUtils
													.isEmpty(defaultTypeitem)&&!defaultTypeitem.equals("null")) {
												int element = Integer
														.valueOf(defaultTypeitem);
												storyDetailListBean
						...
						private void insertPosition(String[] arr, String[] rect,
								String[] square, String[] fourtheearr,
								String[] onetonoearr, String[] arrDefaultType,
								String[] pageId,int insertPosition) {
							int listSize = isCreateListBean.size();
						
							if (listSize > 0) {
								heightItem = isCreateListBean.get(insertPosition);
								itemIndewx = heightItem.getItemIndex();
								// heightItem =
								// isCreateListBean.get(itemIndewx);
							}
...
LogUtil.error("图片地址"+insertPosition+1+i, storyDetailListBean.getPageImg());
								isCreateListBean.add(insertPosition+1+i,storyDetailListBean);
							}
						}
	...
	private void isRecommendViewInitAdapterData(final String yuantu,
		...
		for (int i = 0; i < pageId.length; i++) {
			...
			if (defaultTypeitem.equals("2")) {// 小正方形

				if ((i & 1) != 0) {// 是奇数 & 操作符代替取余操作符号
	...
	private void uploadImage(List<String> temp_imgs) {
		...
		for (int i = 0; i < temp_imgs.size(); i++) {
			...
			if (!TextUtils.isEmpty(photoPath)&& photoPath.length() > 4) {
