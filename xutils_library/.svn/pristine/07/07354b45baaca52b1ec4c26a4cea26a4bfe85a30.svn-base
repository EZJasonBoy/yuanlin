 ...
 */
public class HttpRequest extends HttpRequestBase implements
	...
	public HttpRequest addQueryStringParameter(NameValuePair nameValuePair) {
		uriBuilder.addParameter(nameValuePair.getName(),
	...
	public HttpRequest addQueryStringParams(List<NameValuePair> nameValuePairs) {
		...
		if (isDes()) {
			result = toStrDes3(nameValuePairs);
		} else {
			result = nameValue(nameValuePairs);
		}
	...
	private HttpRequest nameValue(List<NameValuePair> nameValuePairs) {
	 ...
	 * @throws
	 */
	private HttpRequest toStrDes3(List<NameValuePair> nameValuePairs) {
		JSONObject jsonObj = new JSONObject();
		if (nameValuePairs != null) {
			for (NameValuePair nameValuePair : nameValuePairs) {
				try {
					jsonObj.put(nameValuePair.getName(),
							nameValuePair.getValue());
		...
		String des = "";
		try {
			des = Des3.encode(jsonObj.toString());
		...
		uriBuilder.addParameter(PARAM, des);
