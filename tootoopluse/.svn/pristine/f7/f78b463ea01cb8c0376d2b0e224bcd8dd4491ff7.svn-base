...
public class CreateActSecondStepActivity extends BaseActivity implements
	...
	private void createData() {
		if (secondStepStoryBean != null) {
			...
			setAdapterData(secondStepStoryBean,true);
				...
				CommonUtil.xUtilsPostSend(
						...
						requestParamsNet, new RequestCallBack<String>() {
							...
							public void onSuccess(
								...
								setAdapterData(resultData,false);
							}
	...
	private void setAdapterData(SecondStepStoryBean resultData,boolean isEditext) {
		isEditText=isEditext;
		this.secondStepStoryBean=resultData;
		activityName=secondStepStoryBean.getActivityName();
		wishPage = resultData.getStoryList();
		wishPage=CommonUtil.removeDuplicateWithOrder(wishPage);
		...
		boolean isFirst = SharedPreferenceHelper.getFirstGuideCreateViewPager(this);
		if(isFirst){
	...
	*/
	private void initPagerAdapter() {
		if(listFragment.size()>0&&mViewPager!=null){
...
//		String currentActName = getActivityName();//数据永远已上一次的为准
...
//						.toString();// 上一个activityname
			...
			if(currentFragment.mTvPagerCount!=null){
				currentFragment.mTvPagerCount.setText(String.valueOf(currentPosition+1));
			}
			if(currentFragment.mTvCount!=null){
				currentFragment.mTvCount.setText("/"+String.valueOf(listFragment.size()));
			}
...
//			LogUtil.systemlogInfo("跳到第三部", storyIdArray);
			...
			getActivityName();
			secondStepStoryBean.setActivityName(getActivityName());
			if(!TextUtils.isEmpty(activityName)){
